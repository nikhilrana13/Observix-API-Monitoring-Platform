...
export const CreateNewProject = async (req, res) => {
  try {
    ...
    //validation
    const allowedFields = ["name", "baseUrl", "environment"];
...
export const UserAllProjects = async (req, res) => {
  try {
    ...
    return Response(res, 200, "Projects found", {
      projects,
      pagination: {
        totalPages,
        currentPage: page,
...
// toggle project status active and inactive
export const toggleProjectStatus = async (req, res) => {
  try {
    ...
    await project.save();
    return Response(res, 200, `Project ${status} successfully`, { updateStatus:status });
  } catch (error) {
